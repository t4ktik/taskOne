...
class TimesheetController extends Controller
{
    ...
    public function timesheetStore(Request $request)
    {
        ...
        if($project)
        {
            ...
            // Send Mail
            $tArr = [
                'timesheet_project' => $timesheet->project->name,
                'timesheet_task' => $timesheet->task->name,
            ...
            $resp           = Utility::sendEmailTemplate('Create Timesheet', [$timesheet_user->id => $timesheet_user->email], $tArr, $request->project_id);

            // return redirect()->back()->with('success', __('Timesheet Created Successfully!') . (($resp['is_success'] == false && !empty($resp['error'])) ? '<br> <span class="text-danger">' . $resp['error'] . '</span>' : ''));
