<table class="table">
    ...
    @if(isset($seeAsOwner) && $seeAsOwner == true)
        ...
        @else
            @foreach ($timesheetArray as $key => $timesheet)
                <tr>
                    <td colspan="9"><span class="task-name font-weight-700">{{ $timesheet['task_name'] }}</span></td>
                </tr>
                @foreach ($timesheet['dateArray'] as $dateTimeArray)
                    <tr class="timesheet-user text-center">
                        ...
                        @foreach ($dateTimeArray['week'] as $dateSubArray)
                            <td><span class="task-time" data-type="{{ $dateSubArray['type'] }}" data-user-id="{{ $dateTimeArray['user_id'] }}" data-task-id="{{ $timesheet['task_id'] }}" data-date="{{ $dateSubArray['date'] }}" @if(isset($permissions) && in_array('create timesheet',$permissions)) data-ajax-timesheet-popup="true" data-url="{{ $dateSubArray['url'] }}" @endif>{{ $dateSubArray['time'] != '00:00' ? $dateSubArray['time'] : '-' }}</span></td>
                        @endforeach
                        <td><span class="total-task-time">{{ $dateTimeArray['totaltime'] }}</span></td>
                    </tr>
                @endforeach
            ...
            @foreach ($timesheetArray as $key => $timesheet)
                <tr class="text-center">
                    ...
                    @foreach ($timesheet['dateArray'] as $day => $datetime)
                        <td><span class="task-time" data-type="{{ $datetime['type'] }}" data-task-id="{{ $timesheet['task_id'] }}" data-date="{{ $datetime['date'] }}" @if(isset($permissions) && is_array($permissions) && in_array('create timesheet',$permissions)) data-ajax-timesheet-popup="true" data-url="{{ $datetime['url'] }}" @endif>{{ $datetime['time'] != '00:00' ? $datetime['time'] : '-' }}</span></td>
                    @endforeach
